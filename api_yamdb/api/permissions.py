...
class IsSuperUserOrIsAdminOnly(permissions.BasePermission):
    ...
    только суперпользователю Django, админу Django или
    ...
    def has_permission(self, request, view):
        return (
                ...
                and (
                            request.user.is_superuser or request.user.is_staff or request.user.is_admin)
        )
...
class AnonimReadOnly(permissions.BasePermission):
    ...
    (только чтение данных, например GET).
    """
...
class IsSuperUserIsAdminIsModeratorIsAuthor(permissions.BasePermission):
    ...
    суперпользователю Django, админу Django, аутентифицированным пользователям
